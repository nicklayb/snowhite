defmodule Snowhite.Modules.Weather.Server do
  ...
  defp update(%{options: options} = state) do
    ...
    results =
      ...
      |> Task.async_stream(fn
        :weather -> {:weather, current_weather(api_key, params)}
        :forecasts -> {:forecasts, forecast(api_key, params)}
      end)
  ...
  defp current_weather(api_key, params) do
    case Snowhite.Client.OpenWeather.current_weather(api_key, params) do
      {:ok, weather} -> weather
      _ -> nil
    end
  end
  ...
  defp forecast(api_key, params) do
    case Snowhite.Client.OpenWeather.forecast(api_key, params) do
